...
│   │
│   ├── notifier/
│   │   ├── formatter.py        # Formátování zpráv pro Telegram (Markdown)
...
| **PatentsView** | `patents.py` | Týdenně (ned) | Grafen patenty, lookback 90 dní |
| **Google Trends** | `google_trends.py` | Týdenně (ned) | 5 klíčových slov, timeframe 90 dní |
...
| **4-6** | Sledovat | Rutinní press release, obecný výzkum, zprávy o konkurenci |
...
2. **Semantic / Jaccard similarity** (`src/utils/dedup.py`): Před odesláním se title porovná s posledními 6h alertů. Threshold: `DEDUP_JACCARD_THRESHOLD=0.55`.

```
DEDUP_JACCARD_THRESHOLD — Jaccard koeficient
  0.0 = žádný dedup
  0.55 = default (55% shodných signifikantních slov)
  0.8 = pouze téměř identické nadpisy
```

**Jak Jaccard dedup funguje:**
- Z titulků se odstraní stopwords (a, the, is, corp, stock, ...)
- Zbývající tokeny → Jaccard = |průnik| / |sjednocení|
- Stejná zpráva z TickerTick + Google News typicky dosahuje 0.65–0.80 → potlačena
...
| Groq scoring | ✅ Funguje (llama-3.3-70b-versatile) |
| Anomaly detection | ✅ Funguje (DTPKF detekován) |
| Telegram alerts | ✅ Funguje (bot @ZMGrafenBot, chat_id 66087522) |
| Cron pipeline | ✅ Nainstalován |
| Semantic dedup | ✅ Aktivní (Jaccard 0.55) |
| SEC EDGAR | ⚠️ Implementováno, Canadian-only tickery nemusí být na SEC |
| Google Trends | ⚠️ Implementováno, spouští se týdenně (první run v neděli) |
| Patents | ⚠️ Implementováno, spouští se týdenně |
| Daily summary | ✅ Cron nainstalován (20:00 CET), neproběhl live test |
| Weekly report | ✅ Cron nainstalován (neděle 18:00 CET), neproběhl live test |
...
*Dokumentace aktuální k commitu `baf6d8f` · generováno 11. 3. 2026*
